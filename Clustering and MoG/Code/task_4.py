...
X_phonemes_1_2 = np.concatenate((X_phonemes_1,X_phonemes_2))
...
# Write your code here
#phoneme 1 k = 3.
data01 = np.load('data/GMM_params_phoneme_01_k_03.npy', allow_pickle=True)
data01 = dict(enumerate(data01.flatten(), 0))
...
#phoneme 2 k = 3.
data02 = np.load('data/GMM_params_phoneme_02_k_03.npy', allow_pickle=True)
data02 = dict(enumerate(data02.flatten(), 0))
...
for i in range(N_f2):
    for j in range(N_f1):
        #Do predictions, using GMM trained for each phoneme.
        pred1 = get_predictions(mu_01, s_01, p_01, np.array([[j+min_f1, i+min_f2]]))
        pred2 = get_predictions(mu_02, s_02, p_02, np.array([[j+min_f1, i+min_f2]]))

        pred1T = np.sum(pred1, axis=1)
        pred2T = np.sum(pred2, axis=1)
        # Compare these predictions, to classify each point of the grid
        if pred1T< pred2T:
            M[i, j]= 1.0
        else:
