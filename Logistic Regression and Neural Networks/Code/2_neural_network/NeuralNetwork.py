    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        for i in range(len(hidden_deltas)):
            ...
            for j in range(len(output_deltas)):
                delta_j += output_deltas[j]*self.w_out[i,j]

            hidden_deltas[i]= delta_j* sigmoid_derivative(self.y_hidden[i])
            ########################################/
        ...
        for i in range(len(self.y_hidden)):
            for j in range(len(output_deltas)):
                ...
                # self.w_out[i,j] = ...
                weight = self.w_out[i,j] - learning_rate*self.y_hidden[i]*output_deltas[j]
                self.w_out[i,j] =  weight
                ########################################/
        ...
        for i in range(len(inputs)):
            for j in range(len(hidden_deltas)):
                ...
                weight = self.w_hidden[i,j] - learning_rate*inputs[i]*hidden_deltas[j]
                self.w_hidden[i,j] = weight
                ########################################/
